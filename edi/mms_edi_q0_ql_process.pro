...
;
; NAME:
;    mms_edi_q0_ql_process
;
; PURPOSE:
;+
;   Process quality 0 counts from EDI electric field mode.
;
;   Calling Sequence::
;       FILES = mms_edi_q0_ql_process()
;       FILES = mms_edi_q0_ql_process(SC)
;       FILES = mms_edi_q0_ql_process(SC, MODE)
;       FILES = mms_edi_q0_ql_process(SC, MODE, DATE_START)
;       FILES = mms_edi_q0_ql_process(SC, MODE, DATE_START, '')
;       FILES = mms_edi_q0_ql_process(SC, MODE, DATE_START, DATE_END)
;
; :Categories:
;    MMS, EDI, L2, Q0
;
...
;-
pro mms_edi_q0_ql_process, sc, mode, tstart, tend, $
NO_LOG=no_log, $
...
;-----------------------------------------------------
	...
	;-----------------------------------------------------
		for i = 0, nTimes - 1 do begin
			...
			;Process data
			status_out = mms_edi_q0_ql_sdc(sc[k], mode[j], fstart, $
			                               FILE_OUT = file_out)
		...
		;-----------------------------------------------------
			...
			if status_out eq 0 $
				...
				else files[count] = strjoin([sc[k], 'edi', mode[j], 'ql', 'q0', fstart], '_')
